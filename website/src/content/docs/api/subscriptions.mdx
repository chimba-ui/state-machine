...
A `select` re-evaluates on any machine change but only calls its listeners when the selected value actually changes. This makes it safe to use directly in React's `useSyncExternalStore`, with no infinite-loop risk from returning a fresh object on every call.
